...
def _run_incremental(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    if macro_cols:
        ...
        r_macro, lam_macro, r2_macro = run_factor_step(macro_cols, macro_betas, r_size, dynamic_size, macro_dt, universe, ridge_lambda=MACRO_RIDGE)
    else:
    ...
    r_mom,   lam_mom,   r2_mom   = run_factor_step(['vol', 'si_composite', 'idio_mom'], {'vol': vol_df, 'si_composite': si_composite, 'idio_mom': mom_df}, r_quality, dynamic_size, calc_dates, universe, ridge_lambda=RIDGE_LAMBDA)
...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    if incremental:
        ...
        print(f"  Ridge penalty    : {RIDGE_LAMBDA}   Macro ridge: {MACRO_RIDGE}")
    ...
    # Common dates step 6 — uses resid_quality (common dates) as input
    # to ensure strict consistency: quality residuals and mom residuals
    # are computed from the same base on the same dates
