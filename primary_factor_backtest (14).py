...
# Quality factor hyperparameters imported from quality_factor.py
...
def run(Pxs_df: pd.DataFrame,
    ...
    # Step 3: Load quality factor scores from DB cache (fast path)
    print("\n  Loading quality factor scores from DB cache...")
    all_tickers = list(sectors_s.index)
    quality_wide = get_quality_scores(
        calc_dates         = pd.DatetimeIndex(calc_dates),
    ...
    for calc_date in calc_dates:
        if calc_date not in quality_wide.index:
            continue
        scores = quality_wide.loc[calc_date].dropna()
        if scores.empty:
            continue
    ...
    print(f"  Factor loaded for {len(factor_by_date)} dates")
