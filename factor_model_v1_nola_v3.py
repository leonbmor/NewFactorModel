...
RIDGE_GRID_SEC    = [0.1, 0.2, 0.4, 0.75, 1.5, 3.0, 5.0, 10.0, 20.0, 40.0] # sectors: bimodal — OLS or heavy regularization
...
def load_quality_scores(universe: list, calc_dates: pd.DatetimeIndex,
    ...
    try:
        ...
        quality_ff = raw.reindex(all_dates).ffill().reindex(calc_dates).astype(float)
        # Z-score cross-sectionally
...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    resid_full_quality_step, _, _ = run_factor_step(
        factor_cols  = ['quality'],
    ...
    # For dates without quality data, fall back to sector residuals
    missing_quality = resid_sec_for_quality.index.difference(resid_full_quality_step.index)
    if len(missing_quality) > 0:
        print(f"  Quality step skipped for {len(missing_quality)} dates "
              f"(no quality data) — using sector residuals as fallback")
        resid_full_quality = pd.concat([
            resid_sec_for_quality.loc[missing_quality],
            resid_full_quality_step
        ]).sort_index()
    else:
        resid_full_quality = resid_full_quality_step
    dropped_qual = full_dates_quality[~full_dates_quality.isin(resid_full_quality_step.index)]
    if len(dropped_qual) > 0:
        print(f"  Quality residuals: {len(resid_full_quality_step)} dates "
              f"(dropped {len(dropped_qual)} dates)")
    else:
        print(f"  Quality residuals: {len(resid_full_quality_step)} dates — no dates dropped")
    ...
    if not last5_snapshot.empty:
        for dt in last5:
            ...
            # Extract ridge lambdas for macro and sector steps
            ridge_lam_str = ''
            if not lambda_macro.empty and 'ridge_lambda' in lambda_macro.columns                     and dt in lambda_macro.index:
                rl = lambda_macro.loc[dt, 'ridge_lambda']
                if not np.isnan(rl):
                    ridge_lam_str += f'   |   Macro Ridge λ: {rl:.2f}'
            if not lambda_sec.empty and 'ridge_lambda' in lambda_sec.columns                     and dt in lambda_sec.index:
