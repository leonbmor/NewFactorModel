...
MOM_LONG         = 252
RIDGE_GRID_MACRO  = [0.0, 0.1, 0.25, 0.75, 1.5, 3.0, 5.0, 10.0]           # macro: moderate collinearity
RIDGE_GRID_MOM    = [0.0, 0.1, 0.25, 0.75, 1.5, 3.0, 5.0, 10.0, 20.0, 50.0] # vol+SI+mom: high structural collinearity
...
def orthogonalize_char(new_char: pd.Series,
    ...
    cross-sectionally on a single date via OLS.

    Runs: new_char = a + B * prior_chars + residual
    Returns the residual (new_char orthogonal to all prior factors).

    Prior chars dict: {name: DataFrame(dates x tickers)} — same structure
    as char_by_date in run_factor_step. Each is looked up at dt.

    If the regression fails or insufficient overlap, returns new_char as-is.
    """
    # Align new char to valid tickers
    y = new_char.dropna()
    if len(y) < MIN_STOCKS:
        return new_char
    ...
    for name, char_df in prior_chars.items():
        ...
        # For sector dummies the char_df has one column per sector ETF
        # — they're already single-date DataFrames with tickers as index
    ...
    if not X_parts:
        return new_char  # nothing to orthogonalize against
    ...
    try:
        ...
        resid = y_ - X_ @ coeffs
        result = pd.Series(resid, index=valid)
        # Reindex back to original index (keep NaN for missing)
        return result.reindex(new_char.index)
    except Exception:
        return new_char
...
def orthogonalize_char_df(char_df: pd.DataFrame,
                           ...
                           calc_dates: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Apply orthogonalize_char across all dates in calc_dates.
    ...
    for dt in calc_dates:
        ...
        if len(raw) < MIN_STOCKS:
            result[dt] = raw
            continue
        # Build prior_chars_dt: subset each prior to this date
        perp = orthogonalize_char(raw, prior_chars, dt)
        result[dt] = perp
...
def load_quality_scores(universe: list, calc_dates: pd.DatetimeIndex,
    ...
    try:
        ...
        quality_ff = anchor_df.reindex(all_dates).ffill().bfill().reindex(calc_dates).astype(float)
...
def _run_incremental(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    si_perp  = orthogonalize_char_df(si_composite,  prior_for_si,  calc_dates)
    prior_for_vol = dict(prior_for_si); prior_for_vol['si_composite'] = si_perp
    vol_perp = orthogonalize_char_df(vol_df,         prior_for_vol, calc_dates)
    prior_for_mom = dict(prior_for_vol); prior_for_mom['vol'] = vol_perp
    mom_perp = orthogonalize_char_df(mom_df,         prior_for_mom, calc_dates)
    prior_for_val = dict(prior_for_mom); prior_for_val['idio_mom'] = mom_perp
    value_perp = orthogonalize_char_df(value_df,     prior_for_val, calc_dates)
...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    si_perp = orthogonalize_char_df(si_composite, prior_for_si, common_dates)
    ...
    vol_perp = orthogonalize_char_df(vol_df, prior_for_vol, common_dates)
    ...
    # Full history vol residuals for momentum lookback (on full quality residuals)
    resid_full_si, _, _ = run_factor_step(
        factor_cols  = ['si_composite'],
        char_by_date = {'si_composite': si_perp},
        all_rets     = resid_full_quality,
        dynamic_size = dynamic_size,
        calc_dates   = resid_full_quality.index.intersection(si_perp.index),
        universe     = universe,
        ridge_lambda = 0.0,
    )
    resid_full_vol, _, _ = run_factor_step(
        ...
        dynamic_size = dynamic_size,
        calc_dates   = resid_full_si.index.intersection(vol_perp.index),
        universe     = universe,
        ridge_lambda = 0.0,
    )
    ...
    # Orthogonalize momentum vs all prior
    prior_for_mom = dict(prior_for_vol)
    prior_for_mom['vol'] = vol_perp
    ...
    mom_perp = orthogonalize_char_df(mom_df, prior_for_mom, common_dates)
    ...
    value_perp = orthogonalize_char_df(value_df, prior_for_value, common_dates)
    ...
    # Determine most restrictive common stock universe
    ou_stocks      = resid_ou.columns.tolist()
    si_stocks      = [t for t in resid_si_full.columns  if t in resid_ou.columns]
    vol_stocks     = [t for t in resid_vol_full.columns if t in resid_ou.columns]
