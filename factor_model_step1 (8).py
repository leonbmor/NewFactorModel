...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    # --------------------------------------------------------------------------
    # STEP 4: Joint — Idiosyncratic Momentum + 21d Reversal + SI + Quality
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 4: + Idiosyncratic Momentum + 21d Reversal + SI + Quality (Joint)")
    print("=" * 70)
    ...
    print(f"\n  Common sample: {len(common_dates)} dates "
          f"(beta + size + sectors + idio_mom + reversal + SI + quality + value, from st_dt)")
    ...
    # Joint characteristics: idio_mom + reversal + SI + quality all together
    ...
    joint_cols = ['beta', 'size'] + sec_cols + ['idio_mom', 'reversal', 'si_composite', 'quality']

    # Value characteristics: joint + value (last step)
    value_char = {**joint_char, 'value': value_df}
    value_cols = joint_cols + ['value']
    ...
    # Full extended dates → value residuals for DB
    full_dates_value = (full_dates
                        .intersection(value_df.index[value_df.notna().any(axis=1)]))
    resid_full_value, lambda_full_value, _ = run_factor_step(
    ...
    # --------------------------------------------------------------------------
    # STEP 5: + Value (last — after all other factors including momentum)
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 5: + Value Factor (final step)")
    print("=" * 70)

    resid_value, lambda_value, r2_value = run_factor_step(
        factor_cols  = value_cols,
        char_by_date = value_char,
        all_rets     = all_rets,
        dynamic_size = dynamic_size,
    ...
    joint_UFV = variance_stats(resid_joint, "joint_UFV - +IdioMom+Reversal+SI+Quality", sec_UFV)
    _         = variance_stats(resid_joint, "joint_UFV vs UFV",                         UFV)
    value_UFV = variance_stats(resid_value, "value_UFV - +Value (final step)",          joint_UFV)
    _         = variance_stats(resid_value, "value_UFV vs UFV",                         UFV)
    ...
    r2_stats(r2_value,                                    "Full Model + Value")
    ...
    save_lambdas(lambda_value[lambda_value.index.isin(common_dates)], 'factor_lambdas_value')
    ...
    save_residuals(resid_full,       'factor_residuals_joint')  # joint model residuals
    save_residuals(resid_full_value, 'factor_residuals_value')  # full model incl. value
    ...
    print_lambda_summary(lambda_joint, ['beta', 'size', 'idio_mom', 'reversal', 'si_composite', 'quality'],
                         "Full Model (Joint: IdioMom + Reversal + SI + Quality)", common_dates,
                         annual_col='si_composite')
    print_sector_lambdas(lambda_joint, sec_cols, common_dates)
    print_lambda_summary(lambda_value, ['beta', 'size', 'idio_mom', 'reversal', 'si_composite', 'quality', 'value'],
                         "Full Model + Value", common_dates,
                         annual_col='value')
    print_sector_lambdas(lambda_value, sec_cols, common_dates)
    ...
    print_lambda_summary(lambda_joint, ['reversal'],
                         ...
                         annual_col='reversal')
    print_lambda_summary(lambda_joint, ['quality'],
                         "Annual — Quality Factor", common_dates,
                         annual_col='quality')
    print_lambda_summary(lambda_value, ['value'],
    ...
    summary_rows = [
        ...
        ("joint_UFV (+ IdioMom+Rev+SI+Quality)",    joint_UFV, UFV,      sec_UFV),
        ("value_UFV (+ Value)",                     value_UFV, UFV,      joint_UFV),
    ]
    print(f"  {'':42} {'Variance':>12} {'% UFV':>8} {'% prev':>8}")
    print(f"  {'-'*74}")
    for lbl, var, base, prev in summary_rows:
        pct_ufv  = f"{var/base*100:.2f}%"
        pct_prev = f"{var/prev*100:.2f}%" if prev else "---"
        print(f"  {lbl:<42} {var:>12.8f} {pct_ufv:>8} {pct_prev:>8}")
