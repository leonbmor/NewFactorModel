...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    print(f"\n  Common sample: {len(common_dates)} dates "
          f"(beta + size + sectors + idio_mom + SI + quality + reversal + value, from st_dt)")
    ...
    # Step 4 characteristics: idio_mom + SI + quality (no reversal, no value)
    mom_char = {'beta': beta_df, 'size': size_char_df,
                **{col: sec_char[col] for col in sec_cols},
                'idio_mom': mom_df, 'si_composite': si_composite, 'quality': quality_df}
    mom_cols = ['beta', 'size'] + sec_cols + ['idio_mom', 'si_composite', 'quality']

    # Step 5 characteristics: all of step 4 + reversal + value
    full_char = {**mom_char, 'reversal': rev_df, 'value': value_df}
    full_cols = mom_cols + ['reversal', 'value']

    # Full extended dates → step 4 residuals for DB (idio momentum history)
    ...
    # --------------------------------------------------------------------------
    # STEP 5: Reversal + Value (joint, net of momentum)
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 5: + Reversal + Value (joint, net of momentum)")
    print("=" * 70)
    ...
    mom_UFV   = variance_stats(resid_mom,   "mom_UFV   - +IdioMom+SI+Quality",               sec_UFV)
    _         = variance_stats(resid_mom,   "mom_UFV vs UFV",                                UFV)
    joint_UFV = variance_stats(resid_joint, "joint_UFV - +Reversal+Value (net of momentum)", mom_UFV)
    ...
    r2_stats(r2_mom,                                      "Step 4: + IdioMom + SI + Quality")
    r2_stats(r2_joint,                                    "Full Model (Step 5: + Reversal + Value)")
    ...
    save_residuals(resid_full_mom,  'factor_residuals_mom')    # after idio_mom + SI + quality
    ...
    print_lambda_summary(lambda_mom,
                         ['beta', 'size', 'idio_mom', 'si_composite', 'quality'],
                         "Step 4: IdioMom + SI + Quality", common_dates,
                         annual_col='si_composite')
    print_lambda_summary(lambda_joint,
                         ['beta', 'size', 'idio_mom', 'si_composite', 'quality', 'reversal', 'value'],
                         "Full Model (Step 5: + Reversal + Value)", common_dates,
                         annual_col='value')
    ...
    summary_rows = [
        ...
        ("mom_UFV   (+ IdioMom+SI+Quality)",            mom_UFV,   UFV,      sec_UFV),
        ("joint_UFV (+ Reversal+Value)",                joint_UFV, UFV,      mom_UFV),
    ]
