...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    # Step 4: idio_mom + vol + SI + quality (forward-looking / risk signals)
    ...
    # Step 5: step 4 + value only (mean-reversion, net of momentum)
    full_char = {**mom_char, 'value': value_df}
    full_cols = mom_cols + ['value']
    ...
    full_dates = (full_dates_mom
                  .intersection(value_df.index[value_df.notna().any(axis=1)]))
    resid_full, lambda_full, _ = run_factor_step(
        factor_cols  = full_cols,
        char_by_date = full_char,
        all_rets     = all_rets,
        dynamic_size = dynamic_size,
    ...
    # --------------------------------------------------------------------------
    # STEP 5: Value (net of momentum, risk factors)
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 5: + Value (net of momentum + vol + SI + quality)")
    print("=" * 70)
    ...
    joint_UFV = variance_stats(resid_joint, "joint_UFV - +Value (net of momentum+vol)",  mom_UFV)
    ...
    r2_stats(r2_joint,                                    "Full Model (Step 5: + Value)")
    ...
    save_residuals(resid_full_mom,  'factor_residuals_mom')    # after idio_mom + vol + SI + quality
    ...
    print_lambda_summary(lambda_joint,
                         ...
                         "Full Model (Step 5: + Value)", common_dates,
                         annual_col='value')
    print_sector_lambdas(lambda_joint, sec_cols, common_dates)
    # Annual breakdown for each signal
    print_lambda_summary(lambda_mom,   ['idio_mom'],
                         "Annual — Idio Momentum", common_dates,
                         annual_col='idio_mom')
    print_lambda_summary(lambda_mom,   ['vol'],
                         "Annual — Vol Factor", common_dates,
                         annual_col='vol')
    print_lambda_summary(lambda_mom,   ['quality'],
                         "Annual — Quality Factor", common_dates,
                         annual_col='quality')
    print_lambda_summary(lambda_joint, ['value'],
                         "Annual — Value Factor", common_dates,
                         annual_col='value')
    ...
    return {
        ...
        'vol_df':         vol_df,       # vol factor
        'quality_df':     quality_df,   # quality factor
        'value_df':       value_df,     # value factor
        'universe':       universe,
        'sec_cols':       sec_cols,
