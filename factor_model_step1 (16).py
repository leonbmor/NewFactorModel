...
def load_ohlc_tables(universe: list) -> tuple:
    ...
    Tables: daily_open, daily_high, daily_range (high - low = range).
    ...
    range_df = _load_table('daily_range')
    ...
    print(f"  daily_range : {range_df.shape if not range_df.empty else 'EMPTY'}")

    # Check universe coverage
    for name, df in [('daily_open', open_df), ('daily_high', high_df), ('daily_range', range_df)]:
        if not df.empty:
    ...
    if open_df.empty or high_df.empty or range_df.empty:
...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    # --------------------------------------------------------------------------
    # STEP 4: Joint — IdioMom + SI + Quality
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 4: + IdioMom + SI + Quality (Joint)")
    print("=" * 70)
    ...
    # Vol factor from prices (84d EWMA, hl=42)
    # Load OHLC for Garman-Klass vol (falls back gracefully if unavailable)
    ...
    print(f"\n  Common sample: {len(common_dates)} dates "
          f"(beta + size + sectors + idio_mom + vol + SI + quality + value, from st_dt)")
    ...
    # Check which O-U dates already cached
    ...
    # Step 4: idio_mom + vol + SI + quality
    mom_char = {'beta': beta_df, 'size': size_char_df,
                **{col: sec_char[col] for col in sec_cols},
                'idio_mom': mom_df, 'vol': vol_df,
                'si_composite': si_composite, 'quality': quality_df}
    mom_cols = ['beta', 'size'] + sec_cols + ['idio_mom', 'vol', 'si_composite', 'quality']

    # Step 6: step 4 + value
    value_char = {**mom_char, 'value': value_df}
    value_cols = mom_cols + ['value']

    # Full extended dates → step 4 residuals for DB
    full_dates_mom = (valid_ext
                      .intersection(mom_df.index)
                      .intersection(vol_df.index)
                      .intersection(si_composite.index))
    resid_full_mom, _, _ = run_factor_step(
        factor_cols  = mom_cols,
    ...
    # Full extended dates → value model residuals (used for O-U fitting)
    full_dates_value = (full_dates_mom
                        .intersection(value_df.index[value_df.notna().any(axis=1)]))
    resid_full_value, _, _ = run_factor_step(
        factor_cols  = value_cols,
        char_by_date = value_char,
        all_rets     = all_rets,
        dynamic_size = dynamic_size,
    ...
    # Common dates → step 4 variance stats and lambdas
    ...
    # --------------------------------------------------------------------------
    # STEP 6: Value (net of momentum + vol + SI + quality)
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 6: + Value (net of momentum + vol + SI + quality)")
    print("=" * 70)
    ...
    # --------------------------------------------------------------------------
    # STEP 7: O-U Mean Reversion (from value model residuals)
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 7: + O-U Mean Reversion (final step)")
    print("=" * 70)
    ...
    ou_char    = {**value_char, 'ou_reversion': ou_pivot}
    ou_cols    = value_cols + ['ou_reversion']
    ...
    # Restrict step 6 (value) residuals to ou_common for step 7 comparison
    ...
    mom_UFV   = variance_stats(resid_mom,        "mom_UFV   - +IdioMom+Vol+SI+Quality",      sec_UFV)
    ...
    # Step 7 on ou_common (may be slightly smaller sample)
    ...
    r2_stats(r2_mom,   "Step 4: + IdioMom + Vol + SI + Quality")
    r2_stats(r2_joint, "Step 6: + Value")
    r2_stats(r2_ou,    "Step 7: + O-U Mean Reversion")
    ...
    print_lambda_summary(lambda_size,  ['beta', 'size'],
                         "Market Beta + Size", common_dates, annual_col='size')
    ...
    print_lambda_summary(lambda_mom,
                         ['beta', 'size', 'idio_mom', 'vol', 'si_composite', 'quality'],
                         "Step 4: IdioMom + Vol + SI + Quality", common_dates,
                         annual_col='si_composite')
    print_lambda_summary(lambda_joint,
                         ['beta', 'size', 'idio_mom', 'vol', 'si_composite', 'quality', 'value'],
                         "Step 6: + Value", common_dates, annual_col='value')
    print_lambda_summary(lambda_ou,
                         ['beta', 'size', 'idio_mom', 'vol', 'si_composite', 'quality',
                          'value', 'ou_reversion'],
                         "Step 7: + O-U Mean Reversion", ou_common, annual_col='ou_reversion')
    ...
    print_lambda_summary(lambda_mom,   ['quality'],   "Annual — Quality Factor", common_dates, annual_col='quality')
    print_lambda_summary(lambda_joint, ['value'],     "Annual — Value Factor",   common_dates, annual_col='value')
    print_lambda_summary(lambda_ou,    ['ou_reversion'], "Annual — O-U Reversion", ou_common,  annual_col='ou_reversion')
    ...
    summary_rows = [
        ...
        ("mom_UFV   (+ IdioMom+Vol+SI+Quality)",        mom_UFV,   UFV,      sec_UFV),
        ("joint_UFV (+ Value)",                         joint_UFV, UFV,      mom_UFV),
        ("ou_UFV    (+ O-U Mean Reversion)*",           ou_UFV,    UFV_ou,   joint_UFV_ou_scalar),
    ]
    print(f"  {'':46} {'Variance':>12} {'% UFV':>8} {'% prev':>8}")
    print(f"  {'-'*78}")
    for lbl, var, base, prev in summary_rows:
        pct_ufv  = f"{var/base*100:.2f}%"
        pct_prev = f"{var/prev*100:.2f}%" if prev else "---"
        print(f"  {lbl:<46} {var:>12.8f} {pct_ufv:>8} {pct_prev:>8}")
    print("  * O-U computed on ou_common subsample, % prev vs step 6 on same subsample")
