...
def _make_make_params_hash(ic, max_weight, min_weight, zscore_cap, pca_var_threshold,
    ...
    params = dict(
        ...
        qfp=_make_quality_fingerprint(),   # invalidates cache when quality coverage changes
    )
    return hashlib.md5(
...
def run_backtest(
    ...
    try:
        from quality_factor import get_pit_weights_at, _load_pit_weights_cache
        _pit_cached = _load_pit_weights_cache()
        if _pit_cached:
            _first_pit = min(_pit_cached.keys())
            ...
            if _min_start > start_date:
                print(f"  Guardrail: MB_START_DATE adjusted from {start_date.date()} "
                      f"to {_min_start.date()} (first PIT weights: {_first_pit.date()} + 12m)")
