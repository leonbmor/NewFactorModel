...
def calc_gs_components(ticker: str, calc_date: pd.Timestamp) -> dict:
    """
    Re-fetch the 8-quarter revenue series for a ticker at calc_date
    and return the 4 individual YoY symmetric growth rates plus the median.
    Returns dict: {YoY_Q1, YoY_Q2, YoY_Q3, YoY_Q4, GS_median}
    """
    result = {'YoY_Q1': np.nan, 'YoY_Q2': np.nan,
              'YoY_Q3': np.nan, 'YoY_Q4': np.nan}
    try:
        feq = get_historical_feq(ticker, calc_date)
        if not feq:
            return result
        s = get_8q_backward(ticker, feq, 'totalRevenues', 'income_data')
        shifted = s.shift(4)
        denom   = ((s + shifted) / 2).abs().replace(0, np.nan)
        growth  = ((s - shifted) / denom * 100).replace([np.inf, -np.inf], np.nan)
        # Pairs are at positions 4,5,6,7 (Q5vsQ1 ... Q8vsQ4)
        pairs = growth.iloc[4:].values
        for i, v in enumerate(pairs[:4]):
            result[f'YoY_Q{i+1}'] = round(float(v), 1) if not np.isnan(v) else np.nan
    except Exception:
        pass
    return result
...
def show_top_stocks(factor_by_date: dict,
    ...
    # Merge GS from all_metrics
    raw = all_metrics[all_metrics['date'] == chosen].set_index('ticker')[['GS']]
    top = top.join(raw, how='left')

    out = top[[
        'Sector', 'mkt_cap($B)',
        'GS_sn',
        'YoY_Q1', 'YoY_Q2', 'YoY_Q3', 'YoY_Q4', 'GS',
        'P/S_q', 'P/Ee_q', 'P/GP_q',
        'val_q', 'factor'
    ]].copy()
