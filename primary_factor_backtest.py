...
IDIO_MOM_TABLE = 'factor_residuals_mom'
        ...
        conn.execute(text(f"""
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    ALTER TABLE {TARGET_TABLE} ADD COLUMN mkt_cap NUMERIC;
                END IF;
            END $$;
        """))
...
def get_historical_feq(ticker: str, calc_date: pd.Timestamp) -> str:
    ...
    dl_before = row_before[0] if row_before else None
    dl_after  = row_after[0]  if row_after  else None

    if not dl_before or not dl_after:
        return None
    ...
    if snap_before.empty or snap_after.empty:
        return None

    sb = snap_before.set_index('period')['value'].apply(lambda x: round(x / 1e6, 1))
    sa = snap_after.set_index('period')['value'].apply(lambda x: round(x / 1e6, 1))

    common = sorted(set(sb.index) & set(sa.index))
    for period in common:
        if sb[period] != sa[period]:
            return period

    new_periods = sorted(set(sa.index) - set(sb.index))
    return new_periods[0] if new_periods else None
...
def build_factor(metrics_df: pd.DataFrame,
    ...
    sm_gs       = df.groupby('Sector')['GS'].transform('median')
    df['GS_sn'] = df['GS'] - sm_gs
    ...
    result            = df.set_index('ticker')[['factor', 'GS_sn', 'P/S_q', 'P/Ee_q',
                                                 'P/GP_q', 'val_q', 'Sector']]
    result['mkt_cap'] = df['mkt_cap'].values if 'mkt_cap' in df.columns else np.nan
    return result
...
def calc_idio_momentum_score(rebal_date: pd.Timestamp,
                              candidates: pd.Index,
                              resid_pivot: pd.DataFrame) -> pd.Series:
    """
    ...
    cum_resid = resid_pivot.loc[window, valid_stk].sum(axis=0).dropna()
    if len(cum_resid) < 5:
        return pd.Series(np.nan, index=candidates)
...
def run(Pxs_df: pd.DataFrame,
    ...
    if use_mom:
        ...
        vol_vs_input = input("  Volume-scaled idio momentum? (y/n) [default=n]: ").strip().lower()
    ...
    return nav_base, nav_alt, port_base, port_alt


if __name__ == "__main__":
    print("Usage: from primary_factor_backtest import run")
    print("       nav_base, nav_alt, port_base, port_alt = run(Pxs_df, sectors_s)")
    nav_base, nav_alt, port_base, port_alt = run(Pxs_df, sectors_s, force_recalc=False)
