...
RESIDUAL_TABLE     = 'factor_residuals_sec'   # net of market, size, macro, sectors
QUALITY_SCORES_TBL = 'quality_scores_df'      # cached composite scores (date x ticker)

# Optimal hyperparameters (from gridsearch vs factor_residuals_sec)
QF_MAV_WINDOW      = 252   # rate MAV window in trading days
QF_THRESHOLD       = 50    # rate momentum threshold in bps

# ==============================================================================
# PRE-DERIVED WEIGHTS
# Update by running: summary, annual, scores, gqf_w, cqf_w = run(Pxs_df, sectors_s)
# then call update_cached_weights(gqf_w, cqf_w) to print the new constants.
# Set use_cached_weights=True (default) to use these instead of re-deriving.
# ==============================================================================
GQF_WEIGHTS: dict = {}   # populated after first run() — copy from output
CQF_WEIGHTS: dict = {}   # populated after first run() — copy from output
...
# ==============================================================================
# RESIDUALIZATION: loads from factor_residuals_sec (net of market, size, macro, sectors)
# ==============================================================================
...
def _load_resid_cache():
    """Load full factor_residuals_sec into memory as wide pivot (date x ticker)."""
    global _RESID_CACHE
    if _RESID_CACHE is not None:
        return _RESID_CACHE
    print(f"  Loading {RESIDUAL_TABLE} from DB (cached for session)...", flush=True)
    with ENGINE.connect() as conn:
        df = pd.read_sql(text(f"""
            SELECT date, ticker, resid FROM {RESIDUAL_TABLE} ORDER BY date
        """), conn)
    df['date']   = pd.to_datetime(df['date'])
...
def _compute_residuals(Pxs_df: pd.DataFrame,
    ...
    """
    Load forward compounded residuals from factor_residuals_sec for the
    window starting strictly after anchor_date, compound over horizon days.
    Pxs_df, sectors_s, size_s accepted for API compatibility but unused.
    """
    ...
    # Keep only tickers with at least half the window populated
...
def _compute_residuals_ols(Pxs_df: pd.DataFrame,
                        sectors_s: pd.Series,
                        size_s: pd.Series,
                        anchor_date: pd.Timestamp,
                        horizon: int) -> pd.Series:
    """OLS residuals fallback: mkt beta + sector dummies + log(size). Kept for reference."""
...
# ==============================================================================
# QUALITY SCORES CACHE — DB-backed persistence
# ==============================================================================

def _ensure_scores_table():
    """Create quality_scores_df table if it does not exist."""
...
def _load_cached_dates() -> set:
    """Return set of dates already in quality_scores_df."""
    try:
        with ENGINE.connect() as conn:
...
def _save_scores(scores_by_date: dict):
    """Upsert quality scores for new dates into quality_scores_df."""
    ...
    print(f"  Saved {len(df):,} quality score rows ({len(dates)} dates) "
          f"to '{QUALITY_SCORES_TBL}'")


def _load_scores_from_db(calc_dates: pd.DatetimeIndex,
                          universe: list) -> pd.DataFrame:
    """Load quality scores from DB for calc_dates, return wide DataFrame."""
...
def update_cached_weights(gqf_w: dict, cqf_w: dict):
    """
    Print GQF_WEIGHTS and CQF_WEIGHTS in copy-paste format for quality_factor.py.
    Call this after run() to update the hardcoded constants.
    """
...
def get_quality_scores(calc_dates: pd.DatetimeIndex,
                        universe: list,
                        Pxs_df: pd.DataFrame,
                        sectors_s: pd.Series,
    ...
    Loads cached scores from DB, identifies missing dates, computes only those,
    saves back to DB. Never modifies existing cached dates unless force_recompute=True.

    Args:
        calc_dates          : All dates needing scores (e.g. valid_ext).
        universe            : List of tickers.
        Pxs_df              : Price DataFrame (must include USGG10YR).
        sectors_s           : Ticker -> sector mapping.
        use_cached_weights  : If True, use GQF_WEIGHTS/CQF_WEIGHTS constants.
                              If False, re-derive weights via IC analysis (slow).
        force_recompute     : If True, recompute ALL dates (drops existing cache).

    Returns:
        DataFrame (calc_dates x universe) of raw (non-z-scored) quality scores.
        Z-scoring is applied in load_quality_scores() in factor_model_step1.py.
    """
    _ensure_scores_table()
    ...
    if len(dates_to_compute) > 0:
        ...
        if use_cached_weights and GQF_WEIGHTS and CQF_WEIGHTS:
            gqf_w = GQF_WEIGHTS
            cqf_w = CQF_WEIGHTS
            print(f"  Using hardcoded weights: "
                  f"{len(gqf_w)} GQF, {len(cqf_w)} CQF components")
        else:
            print("  Deriving weights from IC analysis (slow)...")
            anchor_dates = load_anchor_dates()
            snapshots_   = load_all_snapshots(anchor_dates)
            gqf_w = derive_weights(snapshots_, sectors_s, Pxs_df, regime='growth')
            cqf_w = derive_weights(snapshots_, sectors_s, Pxs_df, regime='conservative')
            print(f"  Derived: {len(gqf_w)} GQF, {len(cqf_w)} CQF components")

        # Load anchors and rate signal
        anchor_dates  = load_anchor_dates()
        snapshots_    = load_all_snapshots(anchor_dates)
        rate_signal   = compute_rate_signal(Pxs_df, QF_MAV_WINDOW, QF_THRESHOLD)
        anchor_sorted = sorted(snapshots_.keys())
    ...
    # Load full result from DB and return
...
def run(Pxs_df: pd.DataFrame,
    ...
    # Populate DB cache with computed scores
    # scores_by_date is keyed by anchor dates — save all of them
    if verbose:
        print("\n  Saving quality scores to DB cache...")
    _ensure_scores_table()
    _save_scores(scores_by_date)

    # Print weight constants for copy-paste into quality_factor.py
    if verbose:
