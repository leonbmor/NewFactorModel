...
def calc_vol_factor(Pxs_df: pd.DataFrame,
    ...
    Z-scored cross-sectionally per date (higher vol = higher score).
    Expected sign: negative (high vol stocks underperform on risk-adjusted basis).
    """
    print(f"  Calculating vol factor (window={VOL_WINDOW}d, hl={VOL_HL}d)...")
    ...
    for dt in calc_dates:
        ...
        # EWMA variance — exponential weights, most recent has highest weight
        n        = len(rets)
        weights  = np.array([(1 - alpha) ** (n - 1 - i) for i in range(n)])
        weights /= weights.sum()
    ...
    print(f"  Vol factor computed: {len(vol_df)} dates")
    return vol_df
