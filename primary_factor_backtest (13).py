...
2. Builds primary factor: rate-conditioned quality composite (GQF/CQF blend)
   from quality_factor.py — replaces legacy GS_sn / val_q factor
...
Assumptions:
  ...
  - Pxs_df must include 'USGG10YR' column for rate regime signal
...
# Quality factor hyperparameters (gridsearch optimal)
QF_MAV_WINDOW = 252
QF_THRESHOLD  = 15
...
def generate_calc_dates(Pxs_df: pd.DataFrame, step_days: int = STEP_DAYS) -> list:
...
def run_backtest(factor_by_date: dict,
                 ...
                 prefilt_pct: float = 1.0,
                 conc_factor: float = 1.0) -> tuple:
    """
    ...
    for i, rebal_date in enumerate(calc_dates):
        ...
        if rebal_date in factor_by_date:
            ...
            # Pre-filter: keep top prefilt_pct by quality score (factor col)
            ...
            if len(fdf) < top_n:
                ...
                if len(fdf) < top_n:
                    ...
                    # top ceil(N/2) stocks get conc_factor/(conc_factor+1)
                    # bottom floor(N/2) stocks get 1/(conc_factor+1)
        ...
        # Buy-and-hold: entry at rebal_date close, exit at next_date close
        ...
        # Fill nav_series for every date in the period (for drawdown/vol calc)
        px_period     = Pxs_df.loc[period_dates, portfolio]
        stk_daily     = px_period.div(px_start, axis=1) - 1  # cumulative ret from period start
...
def run(Pxs_df: pd.DataFrame,
    ...
    # New options
    rebal_input  = input("  Rebalancing frequency in days (or Enter for default=30): ").strip()
    rebal_freq   = int(rebal_input) if rebal_input else 30

    prefilt_input = input("  Pre-filter fraction by quality score 0<x<=1.0 (or Enter for no filter): ").strip()
    if prefilt_input:
        prefilt_pct = float(prefilt_input)
        if not (0 < prefilt_pct <= 1.0):
            print("  Invalid pre-filter value, defaulting to 1.0 (no filter)")
            prefilt_pct = 1.0
    else:
        prefilt_pct = 1.0
    ...
    if conc_input:
        conc_factor = float(conc_input)
        if conc_factor < 1.0:
            print("  Invalid concentration factor, defaulting to 1.0 (equal weight)")
            conc_factor = 1.0
    else:
        conc_factor = 1.0
    ...
    run_alt      = use_vol or use_mom or mktcap_floor is not None \
                   ...
                   or rebal_freq != 30 or prefilt_pct < 1.0 or conc_factor > 1.0
    ...
    if run_alt:
        label = ' + '.join(filter(None, [
            ...
            f'Rebal={rebal_freq}d'          if rebal_freq != 30      else '',
            f'PreFilt={prefilt_pct:.0%}'    if prefilt_pct < 1.0     else '',
            f'Conc={conc_factor:.1f}x'      if conc_factor > 1.0     else '',
        ]))
        print(f"\n  Enhancements: {label}")
    else:
    ...
    # Step 3: Build quality factor scores for each rebalance date
    print("\n  Building quality factor scores...")
    print(f"  Loading anchor snapshots from DB...")
    ...
    print(f"  Deriving GQF weights...")
    gqf_weights = derive_weights(snapshots, sectors_s, Pxs_df, regime='growth')
    print(f"  Deriving CQF weights...")
    cqf_weights = derive_weights(snapshots, sectors_s, Pxs_df, regime='conservative')

    print(f"  GQF components: {list(gqf_weights.keys())}")
    print(f"  CQF components: {list(cqf_weights.keys())}")

    # For each calc_date, find nearest anchor date and use its quality scores
    # Snap each calc_date to the most recent anchor date <= calc_date
    anchor_sorted = sorted(snapshots.keys())

    factor_by_date = {}
    for calc_date in calc_dates:
        # Snap to nearest anchor <= calc_date
        candidates = [a for a in anchor_sorted if a <= calc_date]
        if not candidates:
            continue
        anchor = candidates[-1]
        ...
        # Build factor DataFrame matching expected structure for run_backtest()
        fdf = scores.rename('factor').to_frame()
        fdf['Sector'] = fdf.index.map(sectors_s)

        # Attach mkt_cap from all_metrics if available for filtering
        date_metrics = all_metrics[all_metrics['date'] == calc_date].set_index('ticker')
        if 'mkt_cap' in date_metrics.columns:
            fdf['mkt_cap'] = fdf.index.map(date_metrics['mkt_cap'])

        fdf = fdf.dropna(subset=['factor', 'Sector'])
        if not fdf.empty:
    ...
    return nav_base, nav_alt, port_base, port_alt, factor_by_date, all_metrics, gqf_weights, cqf_weights, snapshots
...
def show_top_stocks(factor_by_date: dict,
                    ...
                    n: int = 50,
                    ref_date: str = None,
                    gqf_weights: dict = None,
                    cqf_weights: dict = None,
                    snapshots: dict = None) -> pd.DataFrame:
    """
    Show top N ranked stocks with quality factor scoring breakdown.
    ...
    gqf_weights    : dict  — GQF metric weights (optional, for decomposition)
    cqf_weights    : dict  — CQF metric weights (optional, for decomposition)
    snapshots      : dict  — anchor snapshots (optional, for decomposition)

    Returns
    -------
    DataFrame with columns:
        Sector, mkt_cap($B), factor, GQF_score, CQF_score, q, ...top metrics
    """
    ...
    if 'mkt_cap' in top.columns:
        ...
        top['mkt_cap($B)'] = np.nan

    # Add GQF / CQF decomposition if weights and snapshots provided
    if gqf_weights and cqf_weights and snapshots:
        from quality_factor import build_derived_metrics, rank_within_sector

        # Snap to nearest anchor
        anchor_sorted = sorted(snapshots.keys())
        candidates    = [a for a in anchor_sorted if a <= chosen]
        if candidates:
            anchor = candidates[-1]
            snap   = build_derived_metrics(snapshots[anchor])
            sec    = sectors_s.copy()
            sec.index = [str(t).split(' ')[0].strip().upper() for t in sec.index]

            all_metrics_q = list(set(list(gqf_weights.keys()) + list(cqf_weights.keys())))

            from quality_factor import rank_within_sector as _rank
            ranked = _rank(snap, sec, all_metrics_q)
            ...
            top['GQF_score'] = weighted_score(gqf_weights).reindex(top.index)
            top['CQF_score'] = weighted_score(cqf_weights).reindex(top.index)

            # Show top contributing metrics per stock (top 3 GQF metrics)
            top_gqf = sorted(gqf_weights, key=gqf_weights.get, reverse=True)[:5]
            top_cqf = sorted(cqf_weights, key=cqf_weights.get, reverse=True)[:3]
            for m in top_gqf + top_cqf:
                if m in ranked.columns:
                    top[m] = ranked[m].reindex(top.index).round(2)

    # Build output columns
    base_cols = ['Sector', 'mkt_cap($B)', 'factor']
    extra_cols = [c for c in ['GQF_score', 'CQF_score'] if c in top.columns]

    # Top GQF/CQF metric columns if present
    metric_cols = [c for c in top.columns
                   if c not in base_cols + extra_cols + ['mkt_cap', 'Sector', 'factor']
                   and c in (list(gqf_weights.keys()) + list(cqf_weights.keys())
                              if gqf_weights and cqf_weights else [])]

    out_cols = base_cols + extra_cols + metric_cols
    out = top[[c for c in out_cols if c in top.columns]].copy()

    pd.set_option('display.max_rows',     n + 5)
    pd.set_option('display.float_format', '{:.3f}'.format)
    pd.set_option('display.width',        240)
    print(out.to_string())
    print(f"\n  factor = rate-conditioned quality composite (GQF/CQF blend)")
    print(f"  GQF_score = growth quality factor score (0-1 sector rank weighted)")
    print(f"  CQF_score = conservative quality factor score (0-1 sector rank weighted)")
...
if __name__ == "__main__":
    ...
    print("       top50 = show_top_stocks(factor_by_date, all_metrics, sectors_s,")
    print("                               gqf_weights=gqf_w, cqf_weights=cqf_w, snapshots=snapshots)")
