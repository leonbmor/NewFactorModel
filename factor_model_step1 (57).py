...
def _run_incremental(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    r_mom,   lam_mom,   r2_mom   = run_factor_step_optimal_ridge(['vol', 'si_composite', 'idio_mom'], {'vol': vol_df, 'si_composite': si_composite, 'idio_mom': mom_df}, r_quality, dynamic_size, calc_dates, universe, lambda_grid=RIDGE_GRID_MOM, default_lambda=0.2)
...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    if incremental:
        ...
        print(f"  Ridge λ grid mom  : {RIDGE_GRID_MOM}")
    ...
    resid_sec_full, lambda_sec, r2_sec = run_factor_step(
        ...
        dynamic_size = dynamic_size,
        calc_dates   = resid_macro_full.index,   # only dates macro step produced output for
        universe     = universe,
    )
    ...
    # For the quality/momentum chain we need sector residuals covering the full
    # extended history. Build a separate full-history sector residual series:
    # on dates where macro residuals exist, use them as input; on earlier dates
    # where k-fold CV dropped them, fall back to size residuals directly.
    resid_for_sec_full = pd.concat([
        # Dates not covered by macro step: use size residuals as fallback
        run_factor_step(
            factor_cols  = sec_cols,
            char_by_date = {col: sec_char[col] for col in sec_cols},
            all_rets     = resid_size_full,
        ...
        # Dates covered by macro step: use macro residuals (already in resid_sec_full)
        resid_sec_full,
    ]).sort_index()
    ...
    print(f"  Quality residuals: {len(resid_full_quality)} dates "
          f"(full_dates_quality={len(full_dates_quality)}, resid_sec={len(resid_sec_for_quality)})")
    ...
    print(f"  Common sample after idio_mom: {len(common_dates)} dates "
          f"(mom_df has {len(mom_df)} dates, dropped {before_mom - len(common_dates)})")
