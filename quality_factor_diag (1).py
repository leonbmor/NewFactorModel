...
def load_quality_snapshot(date: pd.Timestamp) -> pd.DataFrame:
    """
    Load all quality metric rows for a given date.
    Returns DataFrame with bare tickers as index.
    """
    cols_sql = ', '.join([f'"{m}"' for m in QUALITY_METRICS])
    with ENGINE.connect() as conn:
...
def run_quality_diag(Pxs_df: pd.DataFrame,
    ...
    for d_idx, anchor in enumerate(anchor_dates, 1):
        ...
        # Size for residualization — use 'Size' column from snapshot
        if 'Size' not in snap.columns:
            size_s = pd.Series(1.0, index=snap.index)
        else:
            size_s = snap['Size'].dropna()
