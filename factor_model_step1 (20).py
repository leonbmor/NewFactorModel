...
def run(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    # --------------------------------------------------------------------------
    # STEP 4: Sector Dummies — target: size residuals, features: sector dummies only
    # --------------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  STEP 4: Sector Dummies")
    print("=" * 70)
    ...
    # Update common_dates to include mom
    ...
    # Step 7 (value) residuals restricted to ou_common for step 8 comparison
    ...
    sec_UFV     = variance_stats(resid_sec,     "sec_UFV     - Sector Residuals",          size_UFV)
    ...
    r2_stats(r2_sec[r2_sec.index.isin(common_dates)],          "Step 4: Sectors")
    r2_stats(r2_quality[r2_quality.index.isin(common_dates)],  "Step 5: Quality")
    r2_stats(r2_mom,                                            "Step 6: Vol + SI + IdioMom")
    r2_stats(r2_joint,                                          "Step 7: Value")
    r2_stats(r2_ou,                                             "Step 8: O-U Mean Reversion")
    ...
    print_lambda_summary(lambda_sec,     sec_cols,
                         "Step 4: Sectors", common_dates)
    print_sector_lambdas(lambda_sec, sec_cols, common_dates)
    print_lambda_summary(lambda_quality, ['quality'],
                         "Step 5: Quality Factor", common_dates, annual_col='quality')
    print_lambda_summary(lambda_mom,     ['vol', 'si_composite', 'idio_mom'],
                         "Step 6: Vol + SI + IdioMom", common_dates, annual_col='si_composite')
    print_lambda_summary(lambda_joint,   ['value'],
                         "Step 7: Value", common_dates, annual_col='value')
    print_lambda_summary(lambda_ou,      ['ou_reversion'],
                         "Step 8: O-U Mean Reversion", ou_common, annual_col='ou_reversion')
    ...
    print("  * O-U on ou_common subsample; % prev vs step 7 on same subsample")
